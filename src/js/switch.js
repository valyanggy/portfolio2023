...
import { openItem, closeItem } from './item';
...
const showGrid = () => {
    ...
    .set([DOM.outViewportTitles, DOM.outViewportDescription], {
        opacity: 0,
    }, 0) 
...
const hideGrid = () => {
    ...
    .to([DOM.inViewportTitles, DOM.inViewportDescription], {
        duration: 0.4,
        ease: 'power3.inOut',
        startAt: {opacity: 0},
        opacity: 1
...
const getDOMElements = () => {
    ...
    return {
        ...
        inViewportDescription: inViewportItems.map(item => item.DOM.desc),
        outViewportDescription: outViewportItems.map(item => item.DOM.desc),
        ...
        outContainer: outViewportItems.map(item => item.DOM.container),
    };
}
